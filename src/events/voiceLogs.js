...
if (!oldState.serverMute && newState.serverMute) return channel2.wsend({ embeds: [new MessageEmbed().setColor("RANDOM").setAuthor({ name: client.guilds.cache.get(allah.GuildID).name, iconURL: client.guilds.cache.get(allah.GuildID).iconURL({dynamic:true})}).setFooter({ text: `${moment(Date.now()).format("LLL")}`}).setDescription(`
