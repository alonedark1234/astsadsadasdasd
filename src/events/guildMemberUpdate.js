  ...
  await newMember.guild.fetchAuditLogs({
    ...
    newMember.roles.cache.forEach(async role => {
      if (!oldMember.roles.cache.has(role.id)) {
        const emed = new MessageEmbed()
          ...
          .setColor("RANDOM")
          .setDescription(`Kişinin eklenen ve alınan tüm rollerine bakmak için \`!rollog @Pashq\` komutunu kullanın \n**Rol Eklenen kişi**\n ${hedef} - **${hedef.id}** `)
          .addFields(
    ...
    oldMember.roles.cache.forEach(async role => {
      if (!newMember.roles.cache.has(role.id)) {
        ...
        .setAuthor({ name: hedef.tag, iconURL: hedef.displayAvatarURL({ dynamic: true }) })
        .setColor("RANDOM")
          .setDescription(`Kişinin alınan ve eklenen tüm rollerine bakmak için \`!rollog @Pashq\` komutunu kullanın \n**Rolü Alınan kişi** \n${hedef} - **${hedef.id}**`)
          .addFields(
